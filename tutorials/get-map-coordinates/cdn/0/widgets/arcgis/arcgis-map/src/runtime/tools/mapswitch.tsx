/** @jsx jsx */
import { css, jsx, polished, getAppStore, classNames } from 'jimu-core'
...
export default class MapSwitch extends BaseTool<BaseToolProps, unknown> {
      ...
      return (
        <MultiSourceMapContext.Consumer>
          {({ isShowMapSwitchBtn, dataSourceIds, activeDataSourceId, switchMap }) => (
            this.getContent(isShowMapSwitchBtn, dataSourceIds, activeDataSourceId, switchMap)
          )}
        </MultiSourceMapContext.Consumer>
      )
    } else {
  ...
  getContent = (isShowMapSwitchBtn: boolean, dataSourceIds: string[], activeDataSourceId: string, switchMap) => {
    if (isShowMapSwitchBtn) {
      if (this.props.isMobile) {
        return (
          <div className='exbmap-ui-tool esri-widget--button' onClick={switchMap}>
        ...
        return (
          <div css={this.getStyle()} onClick={switchMap}>
            <div
