  ...
  if (layerDataSourceId) {
    ...
    for (let i = 0; i < jimuLayerViewIds.length; i++) {
      ...
      if (tempJimuLayerView && tempJimuLayerView.layer && tempJimuLayerView.layerDataSourceId === layerDataSourceId &&
        tempJimuLayerView.type === LayerTypes.FeatureLayer) {
      ...
      if (tempJimuLayerView && tempJimuLayerView.layer && tempJimuLayerView.layerDataSourceId === layerDataSourceId &&
        ...
        if ((tempJimuLayerView as JimuFeatureLayerView).view) {
          ...
          loadArcGISJSAPIModules([
            ...
            // eslint-disable-next-line
            let Graphic: typeof __esri.Graphic;
            [Query, FeatureLayer, Graphic] = modules
            const query = new Query()
            query.where = querySQL
            query.outFields = ['*']
            query.returnGeometry = true
            ...
            tempFeatureLayer.load().then(() => {
              tempFeatureLayer.queryFeatures(query).then(featureSet => {
                if (featureSet && featureSet.features && featureSet.features.length > 0) {
...
function startFlash (mapBaseView: __esri.MapView | __esri.SceneView, features: __esri.Graphic[], symbol, Graphic: __esri.GraphicConstructor) {
